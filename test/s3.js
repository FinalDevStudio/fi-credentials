...
describe('Fi Credentials from S3', function () {
  ...
  step('should reject if config.s3 is not an object', function (done) {
    const config = {
      source: 's3'
    };
  ...
  step('should reject if config.s3.bucket is not a string', function (done) {
    const config = {
      source: 's3',
      s3: {}
    };
  ...
  step('should reject if config.s3.apiVersion is not a string', function (done) {
    const config = {
      source: 's3',
      s3: {
